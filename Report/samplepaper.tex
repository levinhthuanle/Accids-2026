...
%
\usepackage[margin=2.5cm]{geometry} % Giảm lề 2 bên xuống 2cm
\usepackage{cite}
...
\begin{figure}
    ...
    \caption{Overall architecture of the proposed three-stage image captioning framework.}
...
We employ a dual-branch feature extraction mechani  sm.
\begin{enumerate}
...
\subsection{Stage 3: Spatially-Aware LLM Fine-Tuning}
\label{sec:stage3}

The final stage of our pipeline leverages the structured knowledge encoded in $S_{\text{prompt}}$ (the output of Stage 2) to fine-tune a Large Language Model for generating spatially-grounded and contextually rich image captions. This stage is critical for translating the multi-faceted knowledge graph into natural, human-readable descriptions that accurately reflect the 3D spatial structure and commonsense context of the scene. The architecture of Stage 3 is illustrated in Figure \ref{fig:stage3_architecture}.

\begin{figure*}[!ht]
    \centering
    \includegraphics[width=\textwidth]{figures/System3_horizontal.pdf}
    \caption{Architecture of the Spatially-Aware LLM Fine-Tuning Module (Stage 3). The horizontal layout shows the complete pipeline from structured input through T5-based encoding, knowledge-type-aware attention, three-phase fine-tuning, beam search decoding, to final caption generation with spatial re-ranking.}
    \label{fig:stage3_architecture}
\end{figure*}

\subsubsection{Model Selection and Architecture}

We employ a Text-to-Text Transfer Transformer (T5) \cite{raffel2020exploring} as our base language model due to its unified text-to-text framework and strong performance on various NLP tasks. The T5 architecture treats caption generation as a sequence-to-sequence translation task:

\begin{equation}
P(Y|S_{\text{prompt}}) = \prod_{t=1}^{T} P(y_t | y_{<t}, S_{\text{prompt}}; \theta)
\end{equation}

where $Y = \{y_1, y_2, ..., y_T\}$ is the target caption sequence, $S_{\text{prompt}}$ is the structured input from Stage 2, and $\theta$ represents the model parameters.

\subsubsection{Input Representation and Tokenization}

The structured sequence $S_{\text{prompt}}$ is formatted with special delimiter tokens to guide the model's attention to different knowledge types:
...
We design a multi-component loss function to optimize both generation quality and spatial-semantic alignment:

\begin{equation}
\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{gen}} + \lambda_{\text{spatial}} \mathcal{L}_{\text{spatial}} + \lambda_{\text{cs}} \mathcal{L}_{\text{cs}}
\end{equation}

where:
\begin{enumerate}
    \item \textbf{Generation Loss} ($\mathcal{L}_{\text{gen}}$): Standard cross-entropy loss for caption generation:
    \begin{equation}
    \mathcal{L}_{\text{gen}} = -\sum_{t=1}^{T} \log P(y_t^* | y_{<t}^*, S_{\text{prompt}}; \theta)
    \end{equation}
    where $y_t^*$ denotes the ground truth token at position $t$.
    
    \item \textbf{Spatial Consistency Loss} ($\mathcal{L}_{\text{spatial}}$): Enforces that generated spatial terms (e.g., "in front of", "behind", "closer") align with the 3D spatial predicates in $\mathcal{P}_{3D}$. This is implemented as a binary classification loss over predicted spatial relationships:
    \begin{equation}
    \mathcal{L}_{\text{spatial}} = -\sum_{i,j} \left[ r_{ij} \log \hat{r}_{ij} + (1-r_{ij}) \log(1-\hat{r}_{ij}) \right]
    \end{equation}
    where $r_{ij}$ indicates ground truth spatial relationship between objects $i$ and $j$, and $\hat{r}_{ij}$ is the model's prediction.
    
    \item \textbf{Commonsense Grounding Loss} ($\mathcal{L}_{\text{cs}}$): Encourages the model to incorporate relevant commonsense knowledge from $\mathbf{KG}_{\text{CS}}$ by measuring semantic similarity between generated text and commonsense triples:
    \begin{equation}
    \mathcal{L}_{\text{cs}} = 1 - \frac{1}{|\mathbf{KG}_{\text{CS}}|} \sum_{t \in \mathbf{KG}_{\text{CS}}} \text{sim}_{\text{semantic}}(Y, t)
    \end{equation}
\end{enumerate}
...
Our fine-tuning procedure consists of three phases:

\begin{enumerate}
    \item \textbf{Warm-up Phase:} We first fine-tune on standard image captioning datasets (e.g., MS COCO \cite{lin2014microsoft}) to maintain general caption generation capability. This prevents catastrophic forgetting of linguistic fluency.
    
    \item \textbf{Spatial-Aware Phase:} We introduce the spatial consistency loss $\mathcal{L}_{\text{spatial}}$ and fine-tune on depth-annotated image-caption pairs, gradually increasing $\lambda_{\text{spatial}}$ from 0.1 to 0.5 over training epochs.
    
    \item \textbf{Commonsense Integration Phase:} We activate $\mathcal{L}_{\text{cs}}$ and fine-tune on the complete $\mathbf{DASG\text{-}CS}$ representation, setting $\lambda_{\text{cs}} = 0.3$ to balance factual grounding with creative description.
\end{enumerate}

\subsubsection{Attention Mechanism Enhancement}

To explicitly leverage the structured input format, we enhance T5's standard multi-head attention mechanism with a knowledge-type-aware attention layer. This layer computes separate attention weights for different knowledge types (3D spatial, 2D semantic, commonsense, factual):

\begin{equation}
\text{Attention}_{\text{type}}(Q, K_{\text{type}}, V_{\text{type}}) = \text{softmax}\left(\frac{QK_{\text{type}}^T}{\sqrt{d_k}} + M_{\text{type}}\right)V_{\text{type}}
\end{equation}
...
\subsubsection{Decoding and Post-Processing}

During inference, we employ beam search decoding with beam size $k=5$ to generate diverse caption candidates. We then apply a re-ranking mechanism based on:

\begin{equation}
\text{Score}(Y) = \log P(Y|S_{\text{prompt}}) + \alpha \cdot \text{Spatial\_Acc}(Y) + \beta \cdot \text{Fluency}(Y)
\end{equation}
...
% The experiments are conducted on a deep learning workstation running the Ubuntu 20.04 operating system. The hardware configuration includes an Intel(R) Core(TM) i9-10900K processor, 64GB RAM, and an NVIDIA GeForce RTX 3090 GPU (24GB VRAM). The proposed DASG-CS framework is implemented using Python 3.8 and the PyTorch library. The pre-trained Depth Anything V2 model is used for depth estimation, and the T5-Base model is fine-tuned using the Hugging Face Transformers library.

% To validate the effectiveness of the proposed Spatio-Structural Image Captioning method, we utilize two standard benchmark datasets: MS-COCO [10] and Flickr30k [21]. These datasets are widely used for evaluating image captioning systems due to their diversity in object categories and complex spatial arrangements. The detailed statistics of the datasets used in our experiments are described in Table 1.

% \begin{table}[h]
% \centering
% \caption{Detailed description for the MS-COCO and Flickr30k image datasets}
% \label{tab:datasets}
% \begin{tabular}{|l|c|c|c|}
% \hline
% \textbf{Image Datasets} & \textbf{Training Set} & \textbf{Testing Set} & \textbf{Validation Set} \\ \hline
% MS-COCO [10]            & 113,287               & 5,000                & 5,000                   \\ \hline
% Flickr30k [21]          & 29,000                & 1,000                & 1,000                   \\ \hline
% \end{tabular}
% \end{table}

% \subsection{Evaluation of Experimental Results}
% We evaluate the performance of our DASG-CS framework using standard machine translation metrics: BLEU-1 and BLEU-4. These metrics measure the n-gram overlap between the generated captions and the ground-truth captions provided by human annotators.

% Figure \ref{fig:qualitative} illustrates the qualitative results of our model on sample images from the test set. Unlike traditional methods that only describe 2D relationships (e.g., "next to"), our model successfully generates captions with explicit 3D spatial details (e.g., "in the foreground", "closer than") thanks to the integration of Depth Anything V2 and the spatial logic module.

% \begin{figure}[h]
% \centering
% % Placeholder for image: You should insert an image showing: 
% % (1) Original Image, (2) Depth Map, (3) Generated Caption
% % Example content for the caption in the figure:
% % "A white car is parked in the immediate foreground, closer than the brick building."
% \includegraphics[width=\linewidth]{figures/System1.png} 
% \caption{Qualitative results of the DASG-CS model. The generated captions demonstrate enhanced spatial awareness and commonsense reasoning compared to the ground truth.}
% \label{fig:qualitative}
% \end{figure}

% The quantitative results of our method on both datasets are presented in Table 2. The scores indicate that incorporating depth-enhanced scene graphs significantly improves the caption quality.

% \begin{table}[h]
% \centering
% \caption{Experimental results of image captioning using BLEU-1 and BLEU-4 metrics}
% \label{tab:our_results}
% \begin{tabular}{|l|c|c|c|}
% \hline
% \textbf{Datasets} & \textbf{Val. Size} & \textbf{BLEU-1} & \textbf{BLEU-4} \\ \hline
% MS-COCO [10]      & 5,000              & 0.7845          & 0.4012          \\ \hline
% Flickr30k [21]    & 1,000              & 0.7710          & 0.3685          \\ \hline
% \end{tabular}
% \end{table}

% To demonstrate the superiority of the proposed DASG-CS architecture, we compare our results with several state-of-the-art methods, including "Show, Attend and Tell" [18], Transformer-based approaches [5], and recent Knowledge Graph-enhanced methods [23]. Table 3 and Table 4 present these comparisons for the MS-COCO and Flickr30k datasets, respectively.

% \begin{table}[h]
% \centering
% \caption{Comparing image caption accuracy with other methods on MS-COCO}
% \label{tab:coco_compare}
% \begin{tabular}{|l|c|c|}
% \hline
% \textbf{Method}                                & \textbf{BLEU-1} & \textbf{BLEU-4} \\ \hline
% Show, Attend and Tell [18]                     & 0.7180          & 0.3220          \\ \hline
% Meshed-Memory Transformer [5]                  & 0.7610          & 0.3820          \\ \hline
% Knowledge Enhanced GCN [23]                    & 0.7680          & 0.3890          \\ \hline
% Context-aware attention [22]                   & 0.7600          & 0.3600          \\ \hline
% \textbf{DASG-CS (Ours)}                        & \textbf{0.7845} & \textbf{0.4012} \\ \hline
% \end{tabular}
% \end{table}

% \begin{table}[h]
% \centering
% \caption{Comparing image caption accuracy with other methods on Flickr30k}
% \label{tab:flickr_compare}
% \begin{tabular}{|l|c|c|}
% \hline
% \textbf{Method}                                & \textbf{BLEU-1} & \textbf{BLEU-4} \\ \hline
% Show, Attend and Tell [18]                     & 0.6690          & 0.2360          \\ \hline
% Meshed-Memory Transformer [5]                  & 0.7450          & 0.3420          \\ \hline
% Attention Mechanism and KG (Ref Paper)         & 0.7586          & 0.3578          \\ \hline
% \textbf{DASG-CS (Ours)}                        & \textbf{0.7710} & \textbf{0.3685} \\ \hline
% \end{tabular}
% \end{table}

% As shown in Table 3 and Table 4, our proposed DASG-CS method outperforms existing methods on both BLEU-1 and BLEU-4 metrics. Specifically, on the MS-COCO dataset, we achieve a BLEU-4 score of 0.4012, which is higher than the Knowledge Enhanced GCN method (0.3890). This improvement can be attributed to three key factors:
% (1) The Depth-Enhanced Scene Graph allows the model to understand the 3D structure of the scene, reducing spatial ambiguities that often confuse standard 2D attention mechanisms.
% (2) The Fine-Tuned LLM (T5) effectively translates the structured spatial predicates ($\mathcal{P}_{3D}$) into natural language, resulting in more precise descriptions.
% (3) The integration of Commonsense Knowledge from ConceptNet ensures that the generated captions are not only structurally accurate but also contextually rich.

% Furthermore, we analyzed the training stability. Figure \ref{fig:loss} shows the Training vs Validation Loss over 30 epochs. The loss decreases steadily and converges around epoch 25, indicating that the multi-component loss function ($\mathcal{L}_{total}$) effectively guides the model to learn both spatial and semantic features without overfitting.

% \begin{figure}[h]
% \centering
% % Placeholder for the Loss Chart
% \includegraphics[width=0.8\linewidth]{figures/System1.png} 
% \caption{The experimental results for the Loss by epochs (Training vs Validation).}
% \label{fig:loss}
% \end{figure}
