...
The first stage of our system focuses on generating a Knowledge Graph (KG) that encodes both the semantic entities and their derived spatial-structural relationships within the input image. 
This initial graph, denoted as the Internal Knowledge Graph ($\mathbf{KG}_{\text{Int}}$), serves as the foundational, image-grounded structure before external knowledge enrichment.
...
\end{enumerate}
This process finalizes the Internal Knowledge Graph ($\mathbf{KG}_{\text{Int}}$), a set of RDF Triples that accurately models the scene's visual and spatial configuration. 
This $\mathbf{KG}_{\text{Int}}$ is then serialized into a TTL file as the output of Stage 1, ready for external knowledge enrichment in Stage 2.


\subsection{Stage 2: DASG-CS Construction and Multi-Source Fusion}
\label{sec:stage2}

The primary objective of Stage 2 is the construction of the Depth- and Commonsense-Augmented Scene Graph ($\mathbf{DASG\text{-}CS}$) by integrating the Internal 
Knowledge Graph ($\mathbf{KG}_{\text{Int}}$) with external knowledge sources. This stage addresses the limitation of purely visual KGs by introducing contextual reasoning capabilities. The architecture for this stage is depicted in Figure \ref{fig:stage2_architecture}.
...
\begin{enumerate}
    \item \textbf{Internal Visual and 3D Spatial Triples ($\mathbf{KG}_{\text{Int}}$):} This is the output from Stage 1, containing  2D semantic triples and our novel $\mathcal{P}_{3D}$ set of qualitative 3D spatial predicates.
...
The final stage of our pipeline uses the structured representation $S_{\text{prompt}}$ to fine-tune a language model capable of producing spatially accurate and contextually rich image captions. This stage transforms multi-level knowledge (3D, 2D, commonsense, factual) into natural and coherent descriptions. The overall architecture is shown in Figure~\ref{fig:stage3_architecture}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{figures/System3_horizontal_cropped.pdf}
    \caption{Architecture of the Spatially-Aware LLM Fine-Tuning Module (Stage 3).}
    \label{fig:stage3_architecture}
\end{figure}

\textbf{Model Selection and Architecture:}
We adopt T5 as our base model due to its unified text-to-text design and strong generalization across NLP tasks. Caption generation is treated as a sequence-to-sequence translation process in which the model maps the structured input into a natural caption.
\subsubsection{Input Representation and Tokenization}

The sequence $S_{\text{prompt}}$ is organized using explicit delimiters that separate knowledge categories and help the model attend to them correctly:
...
This structured formatting guides the model to emphasize spatial predicates for positional descriptions and commonsense facts for functionality or behavior.

\subsubsection{Training Objective}

The model is optimized using a combination of three components:

The model is optimized using a combination of three objectives: (i) a \textbf{caption generation objective} that ensures fluent and coherent output aligned with ground-truth captions, 
(ii) a \textbf{spatial consistency objective} that encourages the model to produce spatial expressions matching the 3D relations encoded in the input, 
and (iii) a \textbf{commonsense grounding objective} that increases the semantic alignment between generated descriptions and the commonsense triples provided from Stage~2.

Two weighting coefficients control the importance of spatial alignment and commonsense grounding during training.

\subsubsection{Fine-Tuning Strategy}

Fine-tuning proceeds in three phases:

\begin{enumerate}
    \item \textbf{Warm-up phase:} train on standard captioning datasets (e.g., MS~COCO) to retain linguistic fluency.
    \item \textbf{Spatial-aware phase:} introduce spatial consistency supervision and gradually increase its weight so the model learns to use accurate positional terms.
    \item \textbf{Commonsense phase:} activate the commonsense objective to integrate functional and causal knowledge into the generated captions.
\end{enumerate}
...
The authors would like to thank the Faculty of Information Technology, Ho Chi Minh
 City University of Industry and Trade, which
